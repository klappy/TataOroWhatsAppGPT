...
describe('generateOrFetchSummary', () => {
  ...
  it('avoids duplicating photo URLs already present', async () => {
    delete global.fetch;
    global.fetch = async () => {
      callCount += 1;
      return {
        ok: true,
        json: async () => ({ choices: [{ message: { content: 'Photos Provided: http://x/images/a.jpg' } }] })
      };
    };
    const env = {
      MEDIA_BUCKET: { list: async () => ({ objects: [{ key: 'a.jpg' }] }) },
    ...
    const res = await generateOrFetchSummary({ env, session, phone: '1', baseUrl: 'http://x' });
    assert.strictEqual(callCount, 1);
    // should not append the photo section again
    const expected = 'Photos Provided: http://x/images/a.jpg';
