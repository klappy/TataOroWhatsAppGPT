export async function handleSummaryRequest(request, env) {
  ...
  if (request.method === "GET" && url.pathname.startsWith("/summary/")) {
    ...
    htmlParts.push(
      ...
      )}</p>${session.summary ? `<p>Summary: ${escapeXml(session.summary)}</p>` : ""}</div>`
    );
