...
│   └── booksy-mcp.js         # MCP server for Booksy integration
...
### 6. **Booksy Integration (MCP Server)**

- Model Context Protocol (MCP) server for service discovery and booking assistance
- Complete catalog of Tata Oro's 14 services across 5 categories
- Direct booking links to Tata's Booksy page with step-by-step instructions
- Smart recommendations based on client type (first-time, returning, specific needs)
- 24/7 availability for service information and pricing
...
- `docs/features/implemented/`: Detailed specs for each implemented feature (Shopify, email, nudges, Booksy integration, etc.)
...
- Vision inputs (images) are added with `image_url` blocks
- Assistant is guided to synthesize, not guess
- System prompt lives in `shared/systemPrompt.js`
- Defines tone, service knowledge, and conversation goals

### Summary Flow

- Assistant produces structured Markdown summaries
- Summaries are stored in session along with shareable URLs
- GPT context includes previous summary and link for natural reference
- No regex-based overrides - GPT handles summary references naturally

### Error Handling

- GPT failures are caught and logged with request context
- Fallback responses provided for API errors
...
```json
{
  ...
  "r2Urls": ["https://wa.tataoro.com/images/..."]
}
```
...
2. **Mutation** - Update history, status flags, and timestamps on each interaction
3. **Persistence** - Store back to KV with 30-day TTL
4. **Cleanup** - Reset clears KV entry and removes R2 objects

### Status Flags

- `progress_status`: `initial` → `photo-received` → `midway` → `summary-ready` → `complete`
- `email_status`: `pending` → `sent` → `failed`
- `shopify_status`: `pending` → `sent` → `failed`
- Boolean flags: `nudge_sent`, `summary_email_sent`

---

## 🖼️ Media Handling Strategy

### R2 Storage Pattern

All media uploads use consistent key format:

```
whatsapp:+{E164PhoneNumber}/{timestamp}-{index}.{extension}
```

Example: `whatsapp:+14335551212/1716120082-0.jpeg`

### Dynamic Discovery

Instead of tracking photo URLs in KV, images are discovered dynamically:
...
- Resistant to race conditions from multiple rapid uploads
- No KV updates required for media tracking
- Full image history always retrievable
- Safe cleanup on session reset
...
### Antifragile Integration Patterns
...
try {
  ...
  await updateSessionStatus("sent");
} catch (err) {
  console.error("[ServiceFail]", err.message);
  await updateSessionStatus("failed");
  // Mark for retry in scheduler
}
```

### Graceful Degradation

- External API failures never break user flow
- Status flags in KV track integration state
- Failed operations marked for retry via scheduler
- User receives helpful message regardless of backend failures

### Timeout & Retry Logic

- Scheduler worker scans for stalled sessions (`progress_status: "midway"` + no activity >2h)
- Failed integrations automatically retried
- WhatsApp nudges sent for inactive consultations
- Email summaries triggered on timeout
...
## 🗓️ Booksy Integration Architecture

### MCP Server Implementation

The Booksy integration uses a Model Context Protocol (MCP) server pattern to provide service discovery and booking assistance:

```js
// workers/booksy-mcp.js
export default {
  async fetch(request, env, ctx) {
    const server = new Server(
      { name: "booksy-mcp", version: "1.0.0" },
      { capabilities: { tools: {} } }
    );

    // 5 tools for complete service discovery
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        { name: "get_services", description: "Get all available services" },
        { name: "get_booking_link", description: "Get booking link with instructions" },
        { name: "search_services", description: "Search services by keyword" },
        { name: "get_business_info", description: "Get business information" },
        { name: "get_service_recommendations", description: "Get personalized recommendations" },
      ],
    }));
  },
};
...
Services are organized into 5 categories with 14 total services:

```js
const SERVICES = {
  consultation: [{ id: "consultation", name: "Diagnóstico capilar", price: 0, duration: 30 }],
  curly: [
    { id: "curly_adventure", name: "Curly Adventure", price: 200, duration: 240 },
    { id: "curly_cut_definition", name: "Curly Cut + Definition", price: 150, duration: 180 },
    { id: "deep_wash", name: "Deep Wash", price: 150, duration: 120 },
  ],
  color: [
    { id: "curly_color", name: "Curly Color Experience", price: 250, duration: 300 },
    { id: "hair_color", name: "Hair color", price: 200, duration: 240 },
  ],
  // ... treatment and special categories
};
```
...
```js
// In whatsapp-incoming.js
const bookingKeywords = ["book", "appointment", "schedule", "services", "price"];
if (bookingKeywords.some((keyword) => userMessage.toLowerCase().includes(keyword))) {
  // Call Booksy MCP tools
  const services = await getServices();
  const recommendations = await getServiceRecommendations(clientType);
}
```

#### Booking Flow

1. **Service Discovery**: User asks about services → MCP returns catalog
2. **Recommendations**: Based on client type (first-time, returning, specific needs)
3. **Booking Link**: Direct link to Tata's Booksy page with instructions
4. **Follow-up**: Integration with existing consultation flow

### Benefits

- **24/7 Availability**: Service information available anytime
- **Consistent Pricing**: Always up-to-date service details
- **Reduced Manual Work**: Automated responses to booking questions
- **Direct Integration**: Seamless with existing WhatsApp flow
- **Personalized**: Tailored recommendations based on client type
...
- ✅ Booksy MCP integration for service discovery and booking assistance
